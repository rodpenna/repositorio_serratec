...
import interfaces.br.com.serratec.Principal;
...
public class PedidoDAO {
	...
	public boolean existePedidoID(int idPedido) {
		ResultSet tabela;
		boolean retorno;
							
		//MONTAGEM DE SQL
		String sql = "select * from marketplace.pedido where idPedido = '" + idPedido +"'";
	...
	public ArrayList<Pedido> localizarPedido (Pedido pedido , int idPedido , int idCliente , Timestamp dataEmissao) {
		...
		if (pedido == null && idPedido ==0 && dataEmissao == null) {
			...
			System.out.println("\nNão existem pedidos para este cliente\n");
			
			Principal.localizarPedidoCliente(conexao);
			}
		}
	...
	public int acharPedidoData (Pedido pedido) {
		...
		sql += " where dataEmissao = '" + pedido.getDataEmissao() +"'";
	...
	public Pedido acharPedidoID (int idPedido) {
		...
		try {
			if (tabela.next()) {
				...
				pedido.setDataEmissao(tabela.getTimestamp("dataEmissao"));////////////////////////////////////////////apagar
